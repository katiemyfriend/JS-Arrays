...
function longer(one, two){
  var arrOne = one.length;
  var arrTwo = two.length;
  if (arrOne > arrTwo){
    return one;
  }
  else return two;
...
function employess(employessList, object) {
  for (var i = 0; i < employessList.length; i++) {
    if (object === employessList[i]) {
      var index = employessList.indexOf(object);
      employessList.splice(index, 1);
    }
  }
  return employessList;
}
employess(devMountainEmployees, cahlan);
...
var user2 = {
  name: 'Sally Seal',
  email: 'sallyseal@seal.com',
  password: 'iLoveSeals',
  userName: 'SallyIsASeal'
};
var user3 = {
  name: 'Fred Fox',
  email: 'fredfox@fox.com',
  password: 'ILoveFoxes',
  userName: 'FredIsAFox'
};
var user4 = {
  name: 'Ella Eagle',
  email: 'ellaeagle@eagle.com',
  password: 'ILoveEagles',
  userName: 'EllaIsAEagle'
};
...
function find(userList, userEmail) {
  for (var i = 0; i < userList; i++) {
    if (userEmail === userList[i]) {
      var index = userList.indexOf(userEmail);
      userList.splice(index, 1);
    }
  }
  return userList;
}
find(users, 'tylermcginnis33@gmail.com');
